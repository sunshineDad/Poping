...
import { formatFilePath, formatToolInput, extractDomain } from '@shared/utils'
...
export function ToolLabel({ toolName, toolInput, workingDirectory, onClick }: ToolLabelProps) {
  const generateLabel = (): React.ReactNode => {
    switch (toolName) {
      case 'Read': {
        ...
        if (offset !== undefined && limit !== undefined) pathWithRange = `${filePath}:${offset},${offset + limit}`
        else if (offset !== undefined) pathWithRange = `${filePath}:${offset}`
        else if (limit !== undefined) pathWithRange = `${filePath}:0,${limit}`
        return (
          <>
  ...
  return (
    <div className={`text-sm font-mono text-foreground mb-1 ${onClick ? 'cursor-pointer' : ''}`} onClick={onClick} aria-label={`Tool: ${toolName}`}>
      {generateLabel()}
    </div>
  )
}
